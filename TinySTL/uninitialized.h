...
namespace tstl
{
    ...
    /**
     * uninitialized_move
     * 把 [firts, last) 上的内容移动到以 result 为起始处的空间, 返回移动结束的位置
     */
    template <class InputIter, class ForwardIter>
    ForwardIter
    ...
    {
         return tstl::move(first, first + n, result);
    }
    ...
    unchecked_uninit_move_n(InputIter first, Size n, ForwardIter result, std::false_type)
    {
        ...
        try
        {
            for (; n > 0; --n ,++cur, ++first)
            {
                tstl::construct(&*cur, tstl::move(*first));
            }
        }
    }
    ...
    {
        return tstl::unchecked_uninit_move_n(first, n, result,
                                             std::is_trivially_copy_assignable<
                                             typename iterator_traits<InputIter>
                                             ::value_type>{});
    }
