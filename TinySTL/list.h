...
// 将 list 反转
    ...
    void list<T>::reverse()
    {
        ...
        {
            tstl::swap(i.node_->prev, i.node_->next);
            i.node_ = i.node_->prev;
        }
        tstl::swap(e.node_->prev, e.node_->next);
    }
...
// 创建结点
    ...
    list<T>::create_node(Args&& ...args)
    {
        ...
        try
        {
            data_allocator::construct(tstl::address_of(p->value), tstl::forward<Args>(args)...);
            p->prev = nullptr;
            p->next = nullptr;
        }
        catch (...)
...
// 销毁结点
    ...
    {
        data_allocator::destroy(tstl::address_of(p->value));
        node_allocator::deallocate(p);
    }
...
// 重载比较操作符
    ...
    bool operator<(const list<T>& lhs, const list<T>& rhs)
    {
        return tstl::lexicographical_compare(lhs.cbegin(), lhs.cend(), rhs.cbegin(), rhs.cend());
    }
